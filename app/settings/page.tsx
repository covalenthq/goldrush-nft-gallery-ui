...
export default function Settings({ params }: { params: { slug: string } }) {
    const { setColor, color, borderRadius, setBorderRadius} = useContext(NftContext);
    const router = useRouter();

    return <Flex direction="column" gap="4" pb="4">
      ...
      <Flex wrap="wrap" gap="2">
        {Object.entries(BORDER_RADIUS).map(([radius, value]) => (
            <div key={radius} style={{ borderRadius: value}} className={`border p-2 cursor-pointer ${radius === borderRadius && "bg-accent"}`} onClick={()=>{
